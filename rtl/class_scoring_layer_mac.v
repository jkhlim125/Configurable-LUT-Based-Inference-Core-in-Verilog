// MAC-style class scoring layer.
// Dataflow mirrors `class_scoring_layer`, including the same feature compression
// into a 4-bit "lut_in" address. The difference is the neuron implementation:
// `class_scoring_neuron_mac` replaces the LUT case-table with MAC-style
// multiply-accumulate scoring.
module class_scoring_layer_mac #(
    ...
    genvar i;
    generate
        for (i = 0; i < NUM_NEURONS; i = i + 1) begin : GEN_MAC_SCORING_NEURONS
            ...
            // Compress two 3-bit features into a 4-bit value.
            // We keep the same compression (LSB bits) as the LUT version.
            assign lut_in = {feat_b[1:0], feat_a[1:0]};

            class_scoring_neuron_mac #(
                .LUT_IN_WIDTH(LUT_IN_WIDTH),
                .SCORE_WIDTH(SCORE_WIDTH)
            ) u_class_scoring_neuron_mac (
                .clk(clk),
                .rst_n(rst_n),
                .in_bits(lut_in),
